...
The layer would need both levels: universal roles for cross-domain queries ("all frames where someone is an Agent") and specialized roles for precision ("all frames where someone is a Player").  PLAYER, BUYER, and AUTHOR would each be meanings in the shared vocabulary, each a specialization of the universal AGENT.  The relationship can be expressed in the vocabulary's own terms: `HYPERNYM { (THEME) = PLAYER, (GOAL) = AGENT }`.  The vocabulary describes itself with the same primitives it uses to describe everything else.  Because PLAYER inherits from AGENT, a query at the universal level would still find chess games alongside authorship assertions and anything else where someone acts intentionally.

The exact relationship between these levels, how specialized roles are created, how inheritance interacts with compound keys, whether a separate specialization mechanism is always needed, is an area of ongoing design.  The requirement is clear: queries must work at both levels of specificity.

### Compound keys and indexing

The title and gloss examples above demonstrate compound keys in action: `(VALUE, ENGLISH)` and `(VALUE, RUSSIAN)` use a language qualifier to distinguish bindings that share the same base role.  The same mechanism extends to any domain: `(VIDEO, MKV, UHD)` and `(VIDEO, MKV, HD)` would distinguish video formats; `(CONFIG, REPLICATION)` and `(CONFIG, RETENTION)` would distinguish policy types.  Every element of a compound key is a grounded sememe, not an arbitrary string.
