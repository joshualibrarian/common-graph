...
The absence of a semantic layer is not an oversight. It is a consequence of how computing was built, and the reasons for that construction were sound.

The POSIX filesystem abstraction emerged in the early 1970s from work at Bell Labs, where the design philosophy was elegant and deliberate: everything is a file, and a file is a sequence of bytes. Ken Thompson and Dennis Ritchie did not omit semantics by accident. They omitted them by principle. A filesystem that imposes structure on its contents is a filesystem that constrains what can be stored in it. The beauty of the byte-stream abstraction is its universality — any data, in any format, from any application, can be stored and retrieved. The price of that universality is that the filesystem understands nothing about what it stores.

The same logic operates at every layer. TCP/IP treats payloads as opaque because the network must carry traffic it cannot anticipate. HTTP treats response bodies as opaque (with only a format hint in Content-Type) because the web must host content its designers never imagined. SQL databases treat column values as typed but semantically opaque because the database engine must serve applications it knows nothing about.

In 1970, this was the right trade-off. Storage was expensive, applications were few, and interoperability was a secondary concern — most data lived its entire life within a single system. The filesystem needed to be fast, reliable, and general. It did not need to understand what it held, because only one program at a time was likely to care.

But the world that made this trade-off reasonable has been transformed. We now live in a world of billions of interconnected devices, where data routinely moves between applications, platforms, languages, and continents. The number of applications that touch a single piece of meaningful data — a contact, a document, a photograph, a medical record — has grown from one to dozens. The cost of having no shared semantic vocabulary is no longer a minor inconvenience. It is the reason we have an entire industry (enterprise integration), an entire field (information retrieval), and an entire class of billion-dollar companies (search engines, data brokers) whose fundamental purpose is to compensate for the fact that data cannot describe itself.

The original designers of these layers understood what they were doing. Thompson and Ritchie were not confused about the difference between bytes and meaning. Berners-Lee, when he designed HTML and HTTP, knew perfectly well that the web had no semantic layer — which is why he spent the next two decades trying to add one (Berners-Lee, Hendler, & Lassila, 2001). The problem is not that the design was wrong for its time. The problem is that we have spent fifty years building an enormous edifice on a foundation that systematically discards the one thing users care about most: what things mean and how they relate.
...
- **CHESS** expects Players (bound to specific sides) and a Moves record
...
This means a predicate like CHESS doesn't declare that it expects "Agent" bindings — it declares that it expects **PLAYER** bindings, because PLAYER carries the additional meaning that CHESS needs. But because PLAYER inherits from AGENT, any query at the universal level still works: "all frames where Fischer is an AGENT" finds chess games, authorship assertions, and anything else where Fischer acts intentionally.

#### Compound keys

Roles can be further qualified. A chess game has two players — both filling the PLAYER role — but they need to be distinguished. Compound semantic keys handle this: (PLAYER, WHITE) and (PLAYER, BLACK) are both PLAYER bindings, qualified by a second meaning that specifies *which* player. A movie might have multiple format variants: (VIDEO, MKV, UHD) and (VIDEO, AVI, HD) are both VIDEO bindings, qualified by container format and resolution. The compound key is a sequence of meanings — role plus qualifiers — that together identify what the binding *is*.

Every element of a compound key is a grounded meaning from the shared vocabulary. WHITE is not a string — it is the sememe for the color white, which acquires its chess-specific significance ("the side that moves first") from the context of the CHESS predicate and the PLAYER role it qualifies. The frame provides the disambiguation, just as it does in natural language. There is no need for a separate "chess-white" sememe — the compound key (PLAYER, WHITE) creates a precise meaning through composition. VIDEO is not a MIME type prefix — it is the meaning "moving visual content." MKV is not a file extension — it is the meaning "Matroska multimedia container format." UHD is not an abbreviation — it is the meaning "ultra-high-definition resolution."
...
An important consequence of this structure: there is no fundamental distinction between "the data" and "the metadata" of a frame. A title's text, a video's master file, a chess game's move log, a document's author — each is a role binding on a frame. The title text fills the NAME role on a TITLE frame. The video file fills a content role on a VIDEO frame. The author fills the AGENT role on an AUTHORED frame. Provenance is a binding. Signatures are bindings. Timestamps are bindings. Everything is a role filled on a predicate.
...
A **chess game**: predicate CHESS, bindings (THEME) = the-game, (PLAYER, WHITE) = Fischer, (PLAYER, BLACK) = Spassky, (MOVES) = move-log. The predicate CHESS expects players (qualified by side) and a move record. The THEME binding identifies *which* game — but it is a role binding like any other, not a privileged structural element.

A **video**: predicate VIDEO, bindings (THEME) = the-movie, (MKV, UHD) = master-file, (MKV, HD) = transcode. Content in different formats is expressed through compound role keys.
...
An item, then, is not a new primitive. It is an emergent structure — what you get when frames accumulate around a shared identity. The book is an item. Tolkien is an item. A chess game is an item. A language is an item.
