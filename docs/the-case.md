...
The predicate is not merely a data template. It is a *behavioral specification*. A predicate can declare how it participates in *parsing* — what syntax it uses, what roles it expects from the token stream, what sub-language it delegates to — and how it *evaluates* — what computation it performs with filled bindings. The predicate is simultaneously the schema, the parser instruction, and the program.

This means the operator `+` is not a hardcoded token in a grammar. It is a sememe — a predicate in the shared vocabulary — that declares: "I am infix, precedence 10, left-associative" and "given two operands, I return their sum." The expression parser reads these declarations from the operator's metadata and applies them. There is no precedence table. There is no grammar rule for `+`. There is a sememe with data and behavior, resolved through the same vocabulary as "create" or "author."

This extends to structural symbols. Parentheses — `(` and `)` — are not reserved syntax. They are sememes that declare structural roles: "I open a group" and "I close a group." The parser reads this declaration the same way it reads any other predicate's contribution. There is, in fact, *no reserved syntax at all* — everything resolves through the shared vocabulary.

The practical consequence is that *any domain can define its own syntax* by defining predicates that carry parsing behavior. A chess notation predicate declares: "I understand algebraic notation — delegate the token stream to me." A regular expression predicate declares: "I parse regex syntax." These are not special-cased in the parser. They are predicates with contributions, resolved through the same mechanism as arithmetic operators, English prepositions, and function calls. The frame primitive reaches all the way into how input is interpreted, not just how data is stored.
...
The argument goes further. Not only are mathematical operations frames — they constitute a *language* with its own grammar. And that grammar is data on the predicates themselves.

Consider: the operator `+` is a sememe. It is not a token in a hardcoded grammar. It lives in the shared vocabulary with a canonical identity (`cg.op:add`), just like the verb "create" or the preposition "to." When the expression parser encounters `+`, it resolves it through the dictionary and asks the sememe: "how do you participate in parsing?" The operator responds: "I am infix, precedence 10, left-associative." The parser reads this declaration and handles precedence accordingly. There is no precedence table. There is no separate expression grammar. There is a sememe with metadata, processed by the same resolution mechanism as natural language tokens.

This insight unifies the parsing of natural language and mathematical expressions into a single pipeline. When a user types `create chess where score > sqrt(9) named rematch`, the input contains English ("create chess"), a mathematical sub-expression ("score > sqrt(9)"), and an auxiliary predicate ("named rematch"). These are not three separate parsing paths. They flow through one tokenizer, one resolver, one pipeline. Each predicate — the verb "create," the operator ">," the function "sqrt," the auxiliary "named" — declares its own parsing behavior. The parser consults each one as it encounters them and routes accordingly.

The language being spoken is *inferred from the tokens themselves*. If most resolved tokens carry an English-language scope, the English parser drives. If the tokens carry expression-language scope (operators, parentheses, functions), the expression parser drives. Mixed input triggers language switching within a single expression. The system does not assume a language — it discovers it, token by token.

This extends to any domain-specific notation. Chess algebraic notation ("e4," "Nf3," "O-O") is a language — a set of predicates with their own parsing rules. When a chess game is focused and the user types "e4," the chess-move predicate's parsing contribution says: "delegate to chess notation parsing." The notation parser produces a MOVE frame: `MOVE { THEME → pawn, SOURCE → e2, GOAL → e4 }`. Same frame primitive, different parsing language, same output.

The practical consequence: mathematical and functional expressions are not a separate system bolted onto the side of a semantic layer. They are frames, using the same predicates and roles as everything else, parsed by the same pipeline, through predicates that carry their own syntax. A spreadsheet cell is a frame whose value is the result of an expression frame. The boundary between "data" and "computation" dissolves the same way the boundary between "data" and "metadata" does — because both are role bindings on predicates, and the frame primitive is expressive enough to carry either. And the boundary between "syntax" and "vocabulary" dissolves too — because syntax is just another property that predicates carry.
...
**A unified parsing pipeline.** If predicates carry their own parsing behavior — operators declare precedence, functions declare grouping, prepositions declare role assignment, domain notations declare delegation — then there is no separate grammar for each input type. There is one pipeline that resolves tokens to predicates, asks each predicate how it participates, and routes accordingly. Natural language, mathematical expressions, chess notation, regular expressions, and any future domain syntax all flow through the same mechanism. The "language" is inferred from the tokens, not assumed. This is not an engineering convenience — it is a structural consequence of predicates carrying behavior. When the syntax IS vocabulary, the parser IS the resolver.
