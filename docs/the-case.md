...
**Liveness without a separate system.** If every item can carry frames, and predicates declare lifecycle policies, then real-time shared presence is not a feature bolted on — it is a structural consequence of the frame primitive. A PRESENT frame asserts "I am in this space." An AVATAR_STATE frame, with a retention policy of LATEST (replace previous, don't persist), carries position and orientation at 60Hz. A TYPING frame carries a typing indicator. These are frames — same predicates, same roles, same subscriptions, same scene rendering — distinguished only by the predicate's declared lifecycle: how long the Library retains them, whether they persist to storage, and whether they require individual signatures or trust the authenticated connection. The PRESENT frame can also carry TOPIC stream bindings for video and audio feeds — Chains that accumulate content-addressed chunks. Three temporal modes (durable, ephemeral, streaming), one frame model. A user "entering" a shared 3D space, a chat room, or a collaborative document is the same action: creating a PRESENT frame on that item. Other participants see it through normal subscriptions. The renderer — 3D, 2D, or text — handles it per fidelity. There is no separate presence service, no buddy list, no "online status" system. Presence is contextual and per-item: you are present *in* something, visible to its participants, and your availability is the set of items you are currently present in. This is how Croquet's (Smith, Kay, Raab, & Reed, 2003) vision of a shared, replicated environment — where every object is inherently collaborative — is realized without requiring a single runtime or 3D interface: the semantic frame absorbs what Croquet needed a custom collaboration protocol (TeaTime) to achieve, because the frame primitive was always rich enough to carry temporal state alongside durable assertions.
