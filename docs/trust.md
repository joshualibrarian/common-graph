...
This is an ordinary relation — a filled frame signed by its author. The signer often *is* the THEME (you assert your own trust), but the signature is separate from the relation body. These are first-class graph data: queryable, versionable, revocable. The trust matrix is computed by walking these edges.
...
```
LIKED_BY { theme: post, agent: alice }     signed by: alice
```

So is a "funny," an "insightful," a "misleading" — any sememe can be a reaction predicate. The relation is a filled frame — `predicate { role: value, ... }` — and the signature tells you who asserted it. These are first-class graph data: queryable by anyone who can see them, filterable by trust policy.
...
```
R1: HAS_COMMENT { theme: picture, target: tony's comment }     signed by: tony
R2: LABELED     { theme: R1, target: spam, reason: "obvious scam" }  signed by: jane
R3: ENDORSED_BY { theme: R2, agent: susan, note: "agreed, clear spam" }  signed by: susan
R4: DISPUTED_BY { theme: R2, agent: bob, note: "it's a real comment" }   signed by: bob
```

Jane marks Tony's comment as spam. Susan endorses Jane's moderation. Bob disputes it. All of these are ordinary filled-frame relations — the signature tells you who asserted each one. Attributable, auditable, and subject to trust evaluation. There is no "moderation queue." There is no "appeals process." There is the graph, and everyone's local trust matrix producing a different view.
