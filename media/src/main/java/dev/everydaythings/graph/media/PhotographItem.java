...
public class PhotographItem extends Item {
    ...
    public static class Photograph {
        ...
        @ItemFrame(predicate = CoreVocabulary.Expects.KEY,
                   fieldAs = @Bind(role = ThematicRole.Topic.KEY,
                                   qualifiers = {FrameBody.TYPE_KEY, CoreVocabulary.Required.KEY, MediaVocabulary.Image.KEY}))
        static final ItemID expectImage = MediaVocabulary.Image.IID;

        // EXPECTS — optional frames
        @ItemFrame(predicate = CoreVocabulary.Expects.KEY,
                   fieldAs = @Bind(role = ThematicRole.Topic.KEY,
                                   qualifiers = {FrameBody.TYPE_KEY, MediaVocabulary.Depicts.KEY}))
        static final ItemID expectDepicts = MediaVocabulary.Depicts.IID;

        @ItemFrame(predicate = CoreVocabulary.Expects.KEY,
                   fieldAs = @Bind(role = ThematicRole.Topic.KEY,
                                   qualifiers = {FrameBody.TYPE_KEY, CoreVocabulary.Place.KEY}))
        static final ItemID expectPlace = CoreVocabulary.Place.IID;

        @ItemFrame(predicate = CoreVocabulary.Expects.KEY,
                   fieldAs = @Bind(role = ThematicRole.Topic.KEY,
                                   qualifiers = {FrameBody.TYPE_KEY, CoreVocabulary.Occasion.KEY}))
        static final ItemID expectOccasion = CoreVocabulary.Occasion.IID;

        @ItemFrame(predicate = CoreVocabulary.Expects.KEY,
                   fieldAs = @Bind(role = ThematicRole.Topic.KEY,
                                   qualifiers = {FrameBody.TYPE_KEY, MediaVocabulary.Captured.KEY}))
    ...
    /** Fresh photograph. */
    public PhotographItem(Librarian librarian) {
        super(librarian);
    }

    /** Hydration constructor. */
    @SuppressWarnings("unused")
    protected PhotographItem(Librarian librarian, Manifest manifest) {
