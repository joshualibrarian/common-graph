...
/**
 * X3DH initial-key-agreement (Signal X3DH, simplified to IK + SPK + EK
 * without a one-time pre-key).
 *
 ...
 *   SK  = HKDF(salt=0^32, ikm=F || DH1 || DH2 || DH3, info="cg.x3dh", length=32)
 * </pre>
 *
...
public final class X3dh {
    ...
    private static byte[] kdf(byte[] dh1, byte[] dh2, byte[] dh3) {
        byte[] ikm = new byte[32 + dh1.length + dh2.length + dh3.length];
        ...
        System.arraycopy(dh3, 0, ikm, off, dh3.length);
