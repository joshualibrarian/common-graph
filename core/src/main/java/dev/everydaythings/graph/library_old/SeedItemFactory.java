...
public final class SeedItemFactory {
    ...
    private static BindingTarget encodeValue(Object value) {
        ...
        if (value instanceof Value v) {
            // Encode the Value's canonical CBOR as bare bytes; the semantic
            // type (Endpoint, Quantity, etc.) lives on the binding's qualifiers
            // in the post-cleanup model, not on the Literal's valueType.
            return Literal.ofBytes(v.encodeBinary(Canonical.Scope.RECORD));
        }
        if (value instanceof Canonical c) {
            byte[] bytes = c.encodeBinary(Canonical.Scope.RECORD);
            return Literal.ofBytes(bytes);
        }
        // Fallback: stringify
        if (value != null) {
