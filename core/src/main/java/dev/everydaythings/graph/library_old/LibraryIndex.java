 ...
 */
public interface LibraryIndex extends AutoCloseable {
    ...
    default void indexFrame(ItemID predicate,
        ...
        if (bindings != null) {
            for (var entry : bindings.entrySet()) {
                if (entry.getValue() instanceof BindingTarget.RefTarget refTarget
                        && !refTarget.isCompound()) {
                    store().put(Column.FRAME_BY_ITEM,
                            Column.FRAME_BY_ITEM.key(refTarget.asItemId(), predicate, bodyHash),
                            storageCidBytes, wtx);
                }
            }
        }
