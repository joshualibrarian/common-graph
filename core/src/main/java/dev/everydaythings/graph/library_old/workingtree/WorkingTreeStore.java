...
public final class WorkingTreeStore implements ItemStore {
    ...
    public ContentID persistManifest(ItemID iid, byte[] record, WriteTransaction wtx) {
        ...
        ContentID vid = new ContentID(Digest.Sha256.digest(body), Digest.Sha256.MULTIHASH_TYPE);
    ...
    public ContentID persistContent(byte[] data, WriteTransaction wtx) {
        ...
        ContentID cid = new ContentID(Digest.Sha256.digest(data), Digest.Sha256.MULTIHASH_TYPE);
    ...
    public Path localComponentPath(CompoundKey key) {
        return localDir().resolve(hex(key.toCanonicalString().getBytes(StandardCharsets.UTF_8)));
    }
    ...
    /** Encode bytes to lowercase hex (no multibase prefix). */
    ...
    /** Decode lowercase hex (no multibase prefix). */
