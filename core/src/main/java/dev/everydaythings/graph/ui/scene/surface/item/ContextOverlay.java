...
public final class ContextOverlay {
    ...
    public static ContextOverlay vocabulary(Item item) {
        ...
        // Component nouns/keys — lower than verbs.
        for (Frame frame : item.frames()) {
            String key = frame.frameKey().toCanonicalString();
            out.add(new Entry(
                    key,
                    key,
                    "component",
    ...
    public static ContextOverlay settings(Item item) {
        ...
        for (VerbEntry verb : item.vocabulary()) {
            ...
            boolean looksLikeSettingVerb =
                    CoreVocabulary.Put.KEY.equals(sememe)
                            || CoreVocabulary.Edit.KEY.equals(sememe)
