...
public final class Literal implements BindingTarget {
    ...
    public <T extends Value> T as(Class<T> type) {
        return Canonical.decodeBinary(payload, type, Scope.RECORD);
    }
     ...
     */
    // TODO: format(Library) — was using OLD LibraryOld.findValueImplementation;
    // restore against the new Library when Value-type implementation registry lands.

    /**
     * Human-readable string representation showing the decoded payload.
     *
     * <p>Uses generic CBOR rendering only.
     */
    @Override
