...
/**
 * REQUEST — "I want something" [Tag 11].
 *
...
public record Request(
    ...
    /**
     * What we're requesting. Subtypes distinguished by field presence.
     */
    public sealed interface Target {
        ...
        static Target fromCbor(CBORObject obj) {
            ...
            // - has "subscribe" → Relations
            // - has "cid" (no "iid") → Content
            // - has "iid" → Item
            if (obj.ContainsKey("subscribe")) {
                return Relations.fromCbor(obj);
            } else if (obj.ContainsKey("cid")) {
                return Content.fromCbor(obj);
            } else if (obj.ContainsKey("iid")) {
                return Item.fromCbor(obj);
            }
            throw new IllegalArgumentException("Unknown target format");
        }
