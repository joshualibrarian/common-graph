...
import dev.everydaythings.graph.network.Protocol;
 ...
 */
public class PeerProtocol implements Protocol {

    private static final Logger log = LogManager.getLogger(PeerProtocol.class);
    ...
    public PeerProtocol(PeerContext context) {
    ...
    @Override
    public void onLink(PeerLink link, PeerContext ctx) {
        log.debug("PeerProtocol.onLink called");
    }
     ...
     *
     * <p>Accepts {@link ProtocolMessage} and filters for {@link PeerMessage} subtypes.
     * Heartbeats are handled at the transport layer and never reach here.
     */
    ...
    private void handleMessage(PeerConnection connection, ProtocolMessage message, ItemID replyTo) {
        if (message instanceof PeerMessage pm) {
        ...
        // Non-peer messages (Heartbeat, Ack, etc.) are ignored here
    }
