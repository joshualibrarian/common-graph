...
public interface Canonical {
    ...
    interface CgTag {
        ...
        // Tag 7: removed. Semantic narrowing of a Literal's value used to ride
        // here as Tag 7 [typeIID, payload]; that pattern conflated encoding
        // with meaning. After cleanup, semantic narrowing lives on binding
        // qualifiers; bare CBOR primitives carry the encoding type natively.
         ...
         *
         * <p>Universal marker for "this CBOR value is a Datum." Used for:
         * <ul>
         *   <li><b>Outer-level bodies</b> — Body and Record at the top of an
         *       encoded blob both wrap as {@code Tag-12 [head, [bindings], …]}.
         *       The decoder strips the tag and dispatches to Body/Record based
         *       on inner array length (2 vs 3) and head ref variant.</li>
         *   <li><b>Inline nested datums</b> — when a binding's target is an
         *       entire sub-datum (expression-tree composition), it's a
         *       {@code Tag-12 [head, [bindings]]} just like an outer body.</li>
         * </ul>
         *
         * <p>Wire-level marker only. The Merkle hash strips the tag (datums
         * hash by structural composition, not encoded bytes), so the same
         * semantic content always produces the same DatumID whether tagged
         * or not. The tag exists for decoder dispatch, not for identity.
         */
        int DATUM = 12;

        // Tag 23: removed. The "inline nested frame" marker has been unified
        // with the outer-body marker under {@link #DATUM} (Tag 12); the same
        // wire shape represents a datum whether it sits at the top of a blob
        // or inside a binding's target.

        // Protocol tags (12-22): message type discrimination for CG and Session
        // protocols. Wire format: [4-byte length][Tag(N, CBOR map)]
        // (Vacated and slated for replacement when peer protocol is ported to the
        // new vocabulary-driven model — see task #37.)
    ...
    enum CborType {
        ...
        TEXT(ValueType.TextType.IID, CBORType.TextString, -1),

        /** CBOR integer → cg.value:integer */
        INTEGER(ValueType.IntegerType.IID, CBORType.Integer, -1),

        /** CBOR boolean → cg.value:boolean */
        BOOLEAN(ValueType.BooleanType.IID, CBORType.Boolean, -1),

        /** CBOR byte string → cg.value:bytes */
        BYTES(ValueType.BytesType.IID, CBORType.ByteString, -1),

        /** CBOR Tag 4 [mantissa, exponent] → cg.value:decimal */
        DECIMAL(ValueType.DecimalType.IID, null, 4),

        /** CBOR Tag 1 (epoch seconds/millis) → cg.value:instant */
        INSTANT(ValueType.InstantType.IID, null, 1),

        /** [numerator, denominator] array → cg.value:rational */
        RATIONAL(ValueType.RationalType.IID, CBORType.Array, -1),

        /** CG Tag 9 [magnitude, unit-iid] → cg.value:quantity */
        QUANTITY(ValueType.QuantityType.IID, null, CgTag.QTY),
        ;
