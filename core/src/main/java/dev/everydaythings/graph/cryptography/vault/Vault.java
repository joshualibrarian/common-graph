...
public interface Vault {
     ...
     * {@link dev.everydaythings.graph.cryptography.algorithm.Signing} via the
     ...
     * dev.everydaythings.graph.cryptography.DoubleRatchet.EncryptedMessage}.
     */
    default dev.everydaythings.graph.cryptography.DoubleRatchet.EncryptedMessage encryptInSession(
    ...
    // Entry-based API (new)
    //
    // Vault entries are typed archetype instances ({@link Identity},
    // {@link Authentication}, {@link Conversation}) carrying the structured
    // bindings each archetype declares.  The vault is the runtime owner of
    // these entries; persistence (file, hardware, etc.) lives behind the
    // implementation.
    //
    // The operation methods above (incept, rotate, sign, openSessionTo, ...)
    // are higher-level wrappers; new entry-aware code can work directly with
    // entries via the methods below.
    // ==================================================================================

    /**
     * The root {@link Identity} entry — the one whose keys derive this vault's
     * Signer IID.  Exactly one root per vault.  Other Identity entries may
     * exist as historical versions (FOLLOWS-chained) when the vault tracks
     * rotation history.
     *
     * <p>Default throws to keep existing implementations green; entry-aware
     * implementations override.
     */
    default Identity rootIdentity() {
        throw new UnsupportedOperationException(
                getClass().getSimpleName() + ".rootIdentity() not yet implemented");
    }

    /**
     * Fetch any vault entry by its {@link EntryId}.  Returns empty if no
     * entry with that id exists in this vault.
     */
    default Optional<VaultEntry> get(EntryId id) {
        throw new UnsupportedOperationException(
                getClass().getSimpleName() + ".get(EntryId) not yet implemented");
    }

    /**
     * The unique entry of the given archetype, or empty if none exists.
     * Convenience for archetypes that are normally singleton (Identity).
     */
    default <E extends VaultEntry> Optional<E> entry(Class<E> archetype) {
        throw new UnsupportedOperationException(
                getClass().getSimpleName() + ".entry(Class) not yet implemented");
    }

    /**
     * The entry of the given archetype whose {@code theme} matches the
     * given context.  Used to look up an Authentication entry for a service
     * or a Conversation for a peer.
     */
    default <E extends VaultEntry> Optional<E> entry(Class<E> archetype, ItemRef theme) {
        throw new UnsupportedOperationException(
                getClass().getSimpleName() + ".entry(Class, ItemRef) not yet implemented");
    }

    /** All entries of the given archetype. */
    default <E extends VaultEntry> List<E> entries(Class<E> archetype) {
        throw new UnsupportedOperationException(
                getClass().getSimpleName() + ".entries(Class) not yet implemented");
    }

    /**
     * Persist the entry's current field state.  Used after the caller
     * mutates an entry's fields (e.g., updating a Conversation's ratchet
     * state after sending a message).
     */
    default void update(VaultEntry entry) {
        throw new UnsupportedOperationException(
                getClass().getSimpleName() + ".update(VaultEntry) not yet implemented");
    }

    /** Remove the entry with the given id.  Returns true if removed. */
    default boolean remove(EntryId id) {
        throw new UnsupportedOperationException(
                getClass().getSimpleName() + ".remove(EntryId) not yet implemented");
    }
}
