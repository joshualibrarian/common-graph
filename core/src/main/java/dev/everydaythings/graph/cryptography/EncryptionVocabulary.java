...
public final class EncryptionVocabulary {
     ...
     * dev.everydaythings.graph.cryptography.Signer Signer}.
     *
     ...
     * implemented in {@link dev.everydaythings.graph.cryptography.DoubleRatchet}:
    ...
    // ==================================================================================
    // Conversation — the per-peer DR session as a vault-entry concept
    // ==================================================================================

    /**
     * PEER — the other party in a two-party relationship; the role-head for
     * binding the IID of the participant other than self.  Used on Conversation
     * vault entries to identify whose session this is.
     */
    @Seed.Item(key = Peer.KEY)
    public static final class Peer {
        public static final String KEY = "cg.role:peer";
        private Peer() {}

        @Seed.Frame(predicate = LexicalVocabulary.Gloss.KEY,
              field = @Seed.Binding(role = ThematicRole.Value.KEY, qualifiers = {Language.English.KEY}))
        static final String englishGloss =
                "the other party in a two-party relationship";

        @Seed.Frame(predicate = LexicalVocabulary.Lexeme.KEY,
              field = @Seed.Binding(role = ThematicRole.Value.KEY, qualifiers = {Language.English.KEY, PartOfSpeech.Noun.KEY, GrammaticalFeature.Lemma.KEY}))
        static final String englishNounLemma = "peer";
    }

    /**
     * CONVERSATION_STATE — the serialized state of an active Double-Ratchet
     * session with a peer: root key, sending and receiving chain keys,
     * ratchet keypair, counters, and skipped-message-keys map.
     *
     * <p>Stored as opaque bytes on a Conversation vault entry.  Survives
     * vault restarts so the session can resume.  Held in software (snapshot +
     * restore) rather than in hardware-protected storage even when other key
     * material is hardware-backed — the threat-model mismatch (DR keys are
     * forward-secret and ephemeral by design) makes the performance cost of
     * token-backed storage unfavorable.
     */
    @Seed.Item(key = ConversationState.KEY)
    public static final class ConversationState {
        public static final String KEY = "cg.role:conversation-state";
        private ConversationState() {}

        @Seed.Frame(predicate = LexicalVocabulary.Gloss.KEY,
              field = @Seed.Binding(role = ThematicRole.Value.KEY, qualifiers = {Language.English.KEY}))
        static final String englishGloss =
                "the serialized state of an active Double-Ratchet session with a peer: "
                        + "root key, chain keys, ratchet keypair, counters, and "
                        + "skipped-message-key map";

        @Seed.Frame(predicate = LexicalVocabulary.Lexeme.KEY,
              field = @Seed.Binding(role = ThematicRole.Value.KEY, qualifiers = {Language.English.KEY, PartOfSpeech.Noun.KEY, GrammaticalFeature.Lemma.KEY}))
        static final String englishNounLemma = "conversation state";
    }
}
