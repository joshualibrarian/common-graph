...
@Implements(ItemOld.KEY)
@ItemSeed(key = ItemOld.KEY)
public class ItemOld {
    ...
    private static class BuiltinKeys {
        static final CompoundKey POLICY = CompoundKey.of(ItemID.fromString(PolicySet.KEY));
    }
    ...
    protected LibrarianOld librarian;

    /** Get the librarian context, or null for siloed/seed items. */
    public LibrarianOld itemLibrarian() {
        return librarian;
    }
     ...
     */
    public java.util.stream.Stream<FrameBodyOld> bareFrameBodies() {
        return frames().bareFrames()
                .map(frame -> frames().getLive(frame.frameKey(), FrameBodyOld.class))
     ...
     */
    protected ItemOld(LibrarianOld librarian) {
        this.librarian = librarian;
     ...
     */
    protected ItemOld(LibrarianOld librarian, ItemID iid) {
        this.librarian = librarian;
        this.iid = Objects.requireNonNull(iid, "iid");
        this.dirty = true;
        state.setOwner(this);
     ...
     */
    protected ItemOld(LibrarianOld librarian, ManifestOld manifest) {
        this.librarian = librarian;
     ...
     */
    protected ItemOld(LibrarianOld librarian, Path path, ItemStore fallbackStore) {
        ...
        if (WorkingTreeStore.exists(path)) {
            ...
            for (FrameOld frame : wts.loadHeadComponents()) {
                frames().add(frame);
            }
     ...
     */
    protected ItemOld(LibrarianOld librarian, ItemStore store) {
     ...
     */
    public Optional<FrameOld> componentFrame(Object componentInstance) {
     ...
     */
    public void endorseFrame(FrameBodyOld body) {
        ...
        frames().add(FrameOld.fromBody(body));
    }
     ...
     */
    public Stream<FrameBodyOld> relations() {
        if (librarian == null) {
            return Stream.empty();
        }
     ...
     */
    public Stream<FrameBodyOld> relations(ItemID predicate) {
        if (librarian == null) {
            return Stream.empty();
        }
     ...
     */
    public List<FrameOld> allFrames() {
        List<FrameOld> result = new ArrayList<>(frames().values());
     ...
     */
    public List<FrameOld> unendorsedFrames() {
        ...
        List<FrameOld> result = new ArrayList<>();
        ...
        library.framesByItem(iid).forEach(ref -> {
            ...
            library.loadFrameBody(ref.bodyHash()).ifPresent(body -> {
                ...
                FrameOld frame =
                        FrameOld.fromBody(body);
                frame.setOwner(this);

                // Load attestation records
                List<FrameRecordOld> records = library.loadRecords(body.hash());
                if (!records.isEmpty()) {
     ...
     */
    public List<FrameOld> unendorsedFrames(ItemID predicate) {
        if (librarian == null) return List.of();
     ...
     */
    public byte[] resolveConfig(FrameOld frame, ItemID qualifier) {
        ...
        CompoundKey itemKey = CompoundKey.of(qualifier);
        Optional<FrameOld> itemFrame = frames().getFrame(itemKey);
        if (itemFrame.isPresent()) {
            FrameOld f = itemFrame.get();
            if (f.body() != null) {
        ...
        if (frame != null && frame.type() != null && librarian != null) {
            Optional<ItemOld> predicateItem = librarian.get(frame.type(), ItemOld.class);
            if (predicateItem.isPresent()) {
                Optional<FrameOld> predFrame =
     ...
     */
    public byte[] resolvePresentation(FrameOld frame) {
     ...
     */
    public byte[] resolveVocabulary(FrameOld frame) {
     ...
     */
    public byte[] resolveGeneralConfig(FrameOld frame) {
    ...
    private void initializeFreshComponents() {
        ...
        for (FrameFieldSpec spec : itemSchema.endorsedFrameFields()) {
            ...
            if (spec.localOnly()) {
                ...
                frame = FrameOld.localResource(spec.frameKey(), spec.type(), spec.identity());
                ...
                if (spec.stream()) {
                    ...
                    frame = FrameOld.stream(spec.frameKey(), spec.type(), List.of(), spec.identity());
                    // alias removed — display names resolved via TokenDictionary
                } else {
                    // Snapshot component: CID computed during commit, use placeholder for now
                    frame = FrameOld.snapshot(spec.frameKey(), spec.type(), null, spec.identity());
                    // alias removed — display names resolved via TokenDictionary
                }
            }
     ...
     */
    public ContentID commit(SignerOld signer) {
     ...
     * @return The new ContentID
     */
    public ContentID commit(SignerOld signer, dev.everydaythings.graph.crypt.EncryptionContext encryptionContext) {
        ...
        var manifestBuilder = ManifestOld.builder()
                .iid(iid)
                .implementation(ManifestOld.javaImplementation(this.getClass()))
                .parents(base != null ? List.of(base) : List.of())
                .state(state);
        if (itemBindings != null) {
            for (var b : itemBindings) manifestBuilder.binding(b);
        }
        ManifestOld manifest = manifestBuilder.build();
     ...
     * </ul>
     *
     * <p>Unlike {@link #commit(SignerOld)}, this method:
     * <ul>
     ...
     */
    private void persistComponent(FrameOld frame, ItemStore targetStore,
     ...
     */
    public ManifestOld generateSeedManifest() {
        // Start edit mode if not already
        if (!editing) {
            edit();
        }
     ...
     */
    public byte[] encodeComponentValue(CompoundKey key) {
    ...
    private void scanAndBindFields(dev.everydaythings.graph.crypt.EncryptionContext encryptionContext) {
        ...
        java.util.function.Consumer<FrameBodyOld> storeFrame = (librarian != null) ? librarian::storeFrame : null;
    ...
    private void storeManifest(ManifestOld manifest) {
    ...
    public java.util.Optional<Class<?>> resolveImplementingClass() {
        ...
        for (FrameOld frame : frames()) {
            ...
            if (live.isPresent() && live.get() instanceof FrameBodyOld body) {
     ...
     */
    public static ItemOld instantiateItem(Class<?> implClass, LibrarianOld lib) {
        try {
            var ctor = implClass.getDeclaredConstructor(LibrarianOld.class);
            ctor.setAccessible(true);
            return (ItemOld) ctor.newInstance(lib);
    ...
    private void resolveEndorsements() {
        ...
        for (dev.everydaythings.graph.frame.FrameEndorsement endorsement : endorsements) {
            try {
                FrameOld frame = resolveEndorsement(endorsement);
                if (frame != null) {
     ...
     */
    private FrameOld resolveEndorsement(dev.everydaythings.graph.frame.FrameEndorsement endorsement) {
        if (librarian == null) {
            // No librarian — can't fetch bodies. Create a minimal frame.
            return new FrameOld(endorsement.key(),
        ...
        FrameBodyOld body = FrameBodyOld.fromCborTree(
        ...
        return FrameOld.fromFrameBody(body, endorsement);
    }
     ...
     */
    @SuppressWarnings("unchecked")
    private Object decodeComponent(FrameOld frame) {
        FrameBodyOld body = frame.body();
     ...
     */
    @SuppressWarnings("unchecked")
    private Object createStreamComponent(FrameOld frame) {
     ...
     */
    private Object resolveReference(FrameOld frame) {
        if (librarian == null) {
            return null;
        }
        ItemID target = frame.body().referenceTargetId();
        Optional<ItemOld> resolved = librarian.get(target, ItemOld.class);
        if (resolved.isEmpty()) {
     ...
     */
    @SuppressWarnings("unchecked")
    private Object openLocalResource(FrameOld frame) {
     ...
     */
    private Optional<byte[]> fetchAndDecrypt(FrameOld frame) {
        FrameBodyOld body = frame.body();
     ...
     */
    @SuppressWarnings("unchecked")
    private Object decodeContent(FrameOld frame, byte[] bytes) {
        ItemID typeId = frame.type();
        // FrameBody entries → decode directly (FrameBody is Canonical, not a Component)
        if (FrameBodyOld.TYPE_ID.equals(typeId)) {
            return Canonical.decodeBinary(bytes, FrameBodyOld.class, Canonical.Scope.RECORD);
        }
     ...
     */
    private Path resolveMountPath(CompoundKey key) {
        Path root = (store != null) ? store.root() : null;
        if (root == null) {
            return null;
        }
     ...
     *
     * <p>Uses {@link ItemSchema#bindFieldsFromTable(ItemOld, EndorsementsTable)} to inject
    ...
    public ItemOld actionNew(
        ...
        @SuppressWarnings("unchecked")
        Class<? extends ItemOld> itemClass = (Class<? extends ItemOld>) this.getClass();
        ...
        ItemOld newItem;
        try {
            Constructor<? extends ItemOld> ctor = itemClass.getDeclaredConstructor(LibrarianOld.class);
            ctor.setAccessible(true);
        ...
        if (name != null && !name.isBlank() && newItem.librarian != null) {
            newItem.librarian.storeFrame(FrameBodyOld.builder(
     ...
     */
    public FrameBodyOld relate(ItemID predicate, Map<ItemID, BindingTarget> bindings) {
        return FrameBodyOld.of(predicate, iid, bindings);
    }
     ...
     * @return A new Item
     */
    public static ItemOld create(LibrarianOld librarian) {
        return new ItemOld(librarian);
    }
