...
/**
 * Compress and decompress {@link Opaque.Compressed}s around {@link Body}
 * (and other datum) values.
 *
 * <p>Compression preserves the structural Merkle hash: the
 * {@code Opaque.Compressed}'s {@code wrappedHash} equals the DatumID of the
 * body it wraps.  Verifying a decompressed body's hash against the cached
 * one is how a consumer detects a forged payload.
 *
...
public final class Compress {
     ...
     * body's DatumID, encodes the body to CG-CBOR bytes, deflates those
     * bytes, returns an Opaque carrying both the hash and the compressed
     * payload.
     */
    public static Opaque.Compressed compress(Body body) {
     ...
     * cached hash; callers wanting that check should compare
     * {@code HashTree.hashOf(decompressed)} against {@code opaque.wrappedHash()}.
     */
    public static Body decompress(Opaque.Compressed opaque) {
     ...
     */
    public static Body decompressAndVerify(Opaque.Compressed opaque) {
        Body body = decompress(opaque);
