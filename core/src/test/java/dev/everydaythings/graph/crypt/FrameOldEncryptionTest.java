...
class FrameOldEncryptionTest {
    ...
    class CommitWithEncryption {
        ...
        void dualCids() {
            ...
            Optional<FrameOld> encrypted = librarian.frames().stream()
                    .filter(e -> e.body().isEncrypted())
                    .findFirst();

            assertThat(encrypted).isPresent();
            FrameOld entry = encrypted.get();
        ...
        void localOnlyNotEncrypted() {
            ...
            Optional<FrameOld> vaultEntry = librarian.frames().stream()
    ...
    class EncryptionPolicyOnConfig {
        ...
        void policyCarriedForward() {
            ...
            Optional<FrameOld> frame = librarian.frames().stream()
                    .filter(e -> e.body().hasContent())
                    .findFirst();
            assertThat(frame).isPresent();
            CompoundKey targetKey = frame.get().frameKey();
            ...
            Optional<FrameOld> afterCommit = librarian.frames().stream()
