...
public class UniMorphReader {
    ...
    private static final Map<String, ItemID> TAG_MAP = Map.ofEntries(
            ...
            // Case
            Map.entry("NOM", GrammaticalFeature.Nominative.IID),
            Map.entry("GEN", GrammaticalFeature.Genitive.IID),
            Map.entry("DAT", GrammaticalFeature.Dative.IID),
            Map.entry("ACC", GrammaticalFeature.Accusative.IID),
            // Gender
            Map.entry("MASC", GrammaticalFeature.Masculine.IID),
            Map.entry("FEM", GrammaticalFeature.Feminine.IID),
            Map.entry("NEUT", GrammaticalFeature.Neuter.IID)
    );
    ...
    /** Tags to ignore (indicative is default, compound genders and language-specific markers). */
    private static final Set<String> IGNORE_TAGS = Set.of(
            "IND",                  // indicative mood (default, not distinctive)
            "MASC+FEM",             // compound genders — not representable as single feature
            "MASC+NEUT",
            "FEM+NEUT",
            "MASC+FEM+NEUT",
            "LGSPEC01"              // language-specific marker
    );
