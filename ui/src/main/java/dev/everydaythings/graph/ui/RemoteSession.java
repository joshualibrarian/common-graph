 ...
 * Inherited as {@code Item#librarian}. For a RemoteSession this is typically a
 * thin anonymous local Librarian ({@link Librarian#anonymous()}) acting as a
 * local cache — fetches fall through to the remote librarian via {@link #parley}
 * on cache miss. The <i>real</i> data lives on the remote librarian on the
 * other end of the {@link RemoteConnection}.
 *
 * <p>UI lifecycle (startUi / requestRender / stopUi) lives on the
 * {@link UiSession} base class shared with {@link LocalSession}; this
 * subclass only differs in its <i>librarian-comm</i> path (Parley + Noise
 * over a tunnel vs. direct in-VM dispatch).
 *
 ...
 */
@Log4j2
public class RemoteSession extends UiSession {
    ...
    public RemoteSession(ItemRef iid,
